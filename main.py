#!/usr/bin/env python3
import os, json, time, threading, queue, signal, re
...
# ======= Config from .env =======
MS_HTTP_TOKEN   = os.getenv("MS_HTTP_TOKEN", "")
...
DEBUG_AUDIO     = bool(int(os.getenv("DEBUG_AUDIO","0")))
...
class AIState:
    ...
    def append_to_target(self, text):
        with self.lock:
            s = (self.target_phrase + " " + (text or "").lower())
...
def push_ai_caption(text: str):
    ...
    if not t:
        return
    try:
        ai_caption_q.put_nowait(t)
    except queue.Full:
        pass
    state.append_to_target(t)
    plan_text_envelope(t)
    try:
        if engine is None:
            engine = pyttsx3.init()
...
def search_and_learn(query):
    ...
    try:
        with DDGS() as ddgs:
            ...
            summary = " ".join([r.get('body','')[:80] for r in results])[:240]
            memory.append({"query": q, "summary": summary, "timestamp": time.time()})
            save_memory()
            state.append_to_target(summary)
            push_ai_caption(f"Learned about {q}: {summary[:60]}...")
    except Exception as e:
        push_ai_caption(f"Error searching: {e}")

def handle_intent_or_ack(text: str):
    t = (text or "").strip()
    # Typed commands: /learn TOPIC | learn TOPIC | /search TOPIC | search TOPIC
    m = re.match(r'^\s*(?:/learn|learn|/search|search)\s+(.+)$', t, flags=re.I)
    if m:
        query = m.group(1).strip()
        search_and_learn(query)
        return
    # Voice phrase fallback
    if "machine spirit, search for" in t.lower():
        query = t.lower().replace("machine spirit, search for", "").strip()
        search_and_learn(query)
        return
    # Simple intents
...
def _handle_signal(sig, frame):
    print("Shutting down… (signal received)")
    shutdown_event.set()
    try:
        if engine:
            engine.stop()
...
def main():
    ...
    threads = [
        threading.Thread(target=display_interface, args=(state, ai_caption_q, talking_event, shutdown_event, push_ai_caption, handle_intent_or_ack), daemon=True),
        ...
        threading.Thread(target=start_server, args=(push_ai_caption, search_and_learn, HTTP_PORT, shutdown_event, MS_HTTP_TOKEN), daemon=True),
        # Network ARP scan needs CAP_NET_RAW; skip by default to avoid spam:
