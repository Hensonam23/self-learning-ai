...
def push_ai_caption(text: str):
    ...
    try:
        ...
        if DEBUG: print(f"Caption queued: {text}")
    except queue.Full:
        ...
        if engine is None:
            ...
            def on_end(name, completed):
                ...
                try:
                    ...
                    pass
                if DEBUG: print("TTS finished; listening re-enabled")
...
def audio_input_worker():
    ...
    try:
        ...
        if DEBUG: print(f"Noise floor: {noise_floor:.4f}  ->  speech gate: {amp_gate:.4f}")
        ...
        while True:
            ...
            if should_cut:
                ...
                if snr < min_snr_for_recog:
                    if DEBUG: print(f"Skipping low-SNR chunk (snr={snr:.2f})")
                    continue
