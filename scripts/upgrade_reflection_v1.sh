...
echo "== Upgrade: reflection journal + auto-proposals =="

mkdir -p scripts systemd data/reflections data/logs proposals

# ----------------------------
# scripts/reflect.py
# ----------------------------
cat > scripts/reflect.py <<'PY'
#!/usr/bin/env python3
...
# ----------------------------
# systemd unit templates (stored in repo)
# ----------------------------
cat > systemd/machinespirit-reflect.service <<'UNIT'
[Unit]
Description=MachineSpirit daily reflection journal

[Service]
Type=oneshot
WorkingDirectory=%h/self-learning-ai
ExecStart=/usr/bin/python3 %h/self-learning-ai/scripts/reflect.py
UNIT

cat > systemd/machinespirit-reflect.timer <<'UNIT'
[Unit]
Description=Run MachineSpirit reflection daily
...
cat > systemd/machinespirit-autopropose.service <<'UNIT'
[Unit]
Description=MachineSpirit auto-propose (creates pending proposals only)

[Service]
Type=oneshot
WorkingDirectory=%h/self-learning-ai
ExecStart=/usr/bin/python3 %h/self-learning-ai/scripts/auto_propose.py
UNIT

cat > systemd/machinespirit-autopropose.timer <<'UNIT'
[Unit]
Description=Run MachineSpirit auto-propose daily (after reflection)
...
# ----------------------------
# install units into user systemd
# ----------------------------
mkdir -p ~/.config/systemd/user
cp -a systemd/machinespirit-reflect.service ~/.config/systemd/user/
cp -a systemd/machinespirit-reflect.timer   ~/.config/systemd/user/
cp -a systemd/machinespirit-autopropose.service ~/.config/systemd/user/
cp -a systemd/machinespirit-autopropose.timer   ~/.config/systemd/user/

systemctl --user daemon-reload
systemctl --user enable --now machinespirit-reflect.timer machinespirit-autopropose.timer

echo "OK: timers enabled (reflect + autopropose)"
